...
//#warning "Unimplemented function: void move_stepper_motor_by_step(int num_steps, int direction)"	// delete this line after implementing
	...
	int i = 0;
	//static unsigned char last_pos = 0b11110000 & 0b00010000;
	if(direction > 0){ // CW
		//GPIO_PORTB_DATA_R = last_pos;
		for(i = 0; i < num_steps/4; i++){
			GPIO_PORTB_DATA_R = 0b00001111 | 0b000000000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R = 0b00001111 | 0b00010000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R = 0b00001111 | 0b00100000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R = 0b00001111 | 0b01000000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R = 0b00001111 | 0b10000000;
		...
		for(i = 0; i < num_steps/4; i++){
			GPIO_PORTB_DATA_R =  0b00001111 | 0b00000000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R =  0b00001111 | 0b10000000;
			timer_waitMillis(5);
			GPIO_PORTB_DATA_R =  0b00001111 | 0b01000000;
		...
		GPIO_PORTB_DATA_R = 0b11110000 | 0b000000000; //STAPH
	}
...
int main(void) {
	...
	shaft_encoder_init();
	int count = 0;
	//REPLACEME = 0b00000011;
	while(1){
	 // loop indefinitely
		uint8_t button = button_getButton();
		//count += read_shaft_encoder();
		move_stepper_motor_by_step(10, count += read_shaft_encoder());
		lcd_printf("Button: %d\nShaft: %d",button, count);
