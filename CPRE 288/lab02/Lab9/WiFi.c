...
int WiFi_start(char *psk) {
	...
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R1; //Turn on GPIO for WiFi command mode
...
uint8_t _sendCommand(uint8_t command, uint8_t* param, uint8_t len) {
	...
	//Send WiFi command
	//TODO use your own uart send char function

	uart_sendChar(command);

	//Send parameter (if applicable)
	if(len > 0) {
		//TODO use your own uart send string command

		sendAString(param, len);
	}
		uart_sendChar(0);
